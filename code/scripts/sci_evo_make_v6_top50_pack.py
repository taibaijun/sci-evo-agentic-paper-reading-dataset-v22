...
def patch_0044(case: dict) -> None:
    ...
    note(case, "v6_targeted_repair: steps4-8 narrowed CalB G2/G3, NMA/RIN, and ETA claims to packet-supported evidence")
...
def patch_0104(case: dict) -> None:
    ...
    note(case, "v6_targeted_repair: step2 removed unsupported LP7 reduced-stability wording and grounded selection details")
...
def write_pack(pack_dir: Path, cases: list[dict], *, label: str) -> None:
    ...
    (pack_dir / "v6_patch_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    (pack_dir / "README.md").write_text(
        f"""# Sci-Evo V6 Hybrid Targeted Repair Pack
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build V6 Top50 and Core16 hybrid-repaired packs.")
    parser.add_argument("--input-dataset", default=r"outputs\submission_top50\dataset.jsonl")
    parser.add_argument("--top50-dir", default=r"outputs\submission_top50_v6_hybrid_repaired")
    parser.add_argument("--core16-dir", default=r"outputs\submission_v6_core16_hybrid_repaired")
    ...
    for case in cases:
        ...
        if d in PATCHERS:
            PATCHERS[d](case)
            note(case, "v6_targeted_repair_applied")
    ...
    write_pack(Path(args.core16_dir), core16, label="Core16 validation subset from the same V6 repaired data")
